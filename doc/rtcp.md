...
3. 依次解析这些候选名，并把 DID info 文档里的 `ip`、`ips`、`all_ip` 追加成候选 IP 列表。
4. 当前实现对候选 IP 的处理是**串行尝试**：按解析结果顺序逐个 `TcpStream::connect(<ip>:<rtcp_port>)`，前一个失败后才尝试下一个。
5. 某个 IP TCP 建连成功后，才为**这一条 attempt** 单独生成 `tunnel_token`、AES key、一次性 `xpub`。
6. 在该 TCP 连接上发送明文 `Hello`。
7. 立即把承载连接包装成 `EncryptedStream`（initiator 方向），等待对端返回 `HelloAck`，并回送 `HelloAckConfirm`。
8. 只有 `HelloAck` / `HelloAckConfirm` 完成后，这条连接才会被注册成 tunnel，标记 `can_direct = true`，并加入 `tunnel_map` 供后续复用。
9. 启动 tunnel 读循环。
...
本节分开描述“当前代码行为”和“建议中的演进方向”。前者已经落地，后者还没有实现。
...
如果 DID info 文档里有 `ip`、`ips` 或 `all_ip`，也会作为额外候选 IP 来源。

当前代码的选择逻辑有几个关键特征：

- 解析名按上面的固定顺序去重后依次执行。
- 所有解析得到的 IP 也只做去重，不做本地 RTT 排序、scope 排序或网络环境打分。
- 连接阶段是**串行尝试**：某个 IP 的 TCP connect、`Hello` 发送或 `HelloAck` / `HelloAckConfirm` 失败后，才继续下一个 IP。
- 每条 attempt 都会重新生成一组独立的 `tunnel_token`、AES key、一次性 `xpub`；不会跨 attempt 复用握手材料。
- 当前没有“按 `(DID, remote_ip, network_fingerprint)` 记忆 RTT”的本地缓存，因此所谓“先近后远”在代码里还没有形成稳定、可度量的策略。

这意味着：今天的实现已经支持“多候选名、多候选 IP、失败后继续尝试”，但它还不是 Happy Eyeballs 风格的并发竞速。

### 10.2 推荐的演进方向

结合 RFC 8305（Happy Eyeballs v2）、RFC 6724（地址选择）和 WebRTC ICE（RFC 8445）的实践，Tunnel 连接部分后续更合理的目标模型应是“**排序 + 并发竞速 + 首个协议层验证成功者获胜**”，而不是单纯的“先近后远串行尝试”。

建议的设计约束如下：

- 先对候选地址排序，但排序只决定**启动顺序**，不等于“必须等前一个完全失败”。
- 连接发起采用 staggered parallel attempts：
  - 先发一条首选地址族的 attempt。
  - 之后按固定延迟继续发起下一条 attempt。
  - 默认延迟可先取 `250ms`，弱网下再调优。
- 任意一条 attempt 在**协议层完成验证**后立即获胜，并取消其他 in-flight attempt。
- “获胜”的判定点不应是 TCP connect 成功，而应是：
  1. TCP connect 成功
  2. `Hello` 已发送
  3. `HelloAck` 已收到且可解密
  4. `HelloAckConfirm` 已回送 / 已校验
- 如果未来要实现“先近后远”，建议把“近”定义成可度量的本地策略，而不是模糊描述。更合理的排序因子是：
  - 本地接口/路由可达性
  - 地址 scope 是否匹配
  - 历史应用层 RTT
  - IPv6/IPv4 家族偏好
...
其中，**本地历史 RTT 应高于服务端给定顺序**。服务端只能表达“它认为的优先级”，但最终哪条路径对当前客户端最快，只能由客户端自己的历史观测决定。

如果后续补本地缓存，建议至少把缓存语义定义清楚：

- cache key：`(remote_did, remote_ip, network_fingerprint)`
- `network_fingerprint` 至少要能区分“换了网络环境”这件事；否则旧 RTT 会误导新网络下的选择
- 缓存内容：
  - 最近一次成功时间
  - EWMA 平滑后的 Hello RTT
  - 连续失败计数
...
### 10.3 与 RTCP Hello 流程的结合

RTCP 与普通 TCP 多地址连接还有一个额外要求：最终要选择的是“**能完成 RTCP Hello 握手**”的路径，而不是“仅能建立 TCP 三次握手”的路径。

因此，多 IP 竞速如果未来实现，推荐把 RTT 统计口径统一成 **Hello 往返时间**，而不是 TCP SYN/SYN-ACK 时间：

1. 发起多个地址的 TCP connect attempt。
2. 某条连接建连后立即发送 `Hello`。
3. 只有当 `HelloAck` / `HelloAckConfirm` 完成，这条路径才算“已验证 tunnel”。
4. 记录本次 Hello RTT，用于下一次排序。
5. 取消所有尚未完成验证的其他 attempt。

这种定义与当前 §14.2 的 key-confirmation 机制天然一致，也更符合 ICE 那种“以端到端 connectivity check 结果决定 candidate 胜负”的思路。
