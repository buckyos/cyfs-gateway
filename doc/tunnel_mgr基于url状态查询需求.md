...
> - 阶段四（Selector 集成、控制面 `/tunnels/probe` HTTP API）：尚未实现，仅库内能力可用。
> - 详细差异与未尽项见 §2 和 §15。
...
> 本节描述本需求落地后 `cyfs-gateway-lib` 的当前实际状态；与最初的"实现边界"相比，绝大多数缺口已经补齐。下面同时列出未尽事项，便于后续 Selector/控制面对接时核对。
...
> 进度（截至 2026-04-29）：阶段一/二/三已完成；阶段四（Selector 集成、控制面 HTTP API）尚未实现。
...
### 阶段三：本地连接类协议接入（库内能力已完成；控制面 HTTP API 待补）
