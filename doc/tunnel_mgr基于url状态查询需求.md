...
本文基于 `/Users/liuzhicong/project/buckyos/doc/arch/gateway/服务的多链路选择.md` 的分层设计，并结合当前 `cyfs-gateway` tunnel 框架实现，整理 `tunnel_mgr` 基于 Tunnel URL 的状态查询基础设施需求和当前落地状态。
...
- `src/components/cyfs-gateway-lib/src/tunnel_url_status.rs`（新增：状态模型、prober trait、URL 规范化与排序）
...
>
> - 阶段一（框架/URL history/缓存/落盘/排序/in-flight 合并/LRU）：已完成。
> - 阶段二（RTCP prober、ping_rtt RTT 测量、keep_tunnel 写回、业务建链回写、tunnel-level 事件传播）：已完成。
> - 阶段三（tcp/ptcp/tls/quic/socks 的轻量连接 probe；udp 走 `Unsupported`）：已完成。
> - 阶段四（Selector/forward 集成、控制面 `/tunnels/probe` API）：已部分完成。控制面批量 probe API 已接入；`forward least_time` 已通过 `apply_least_time_via_tunnel_mgr()` 使用 URL history 做受限预算 RTT 重排；后台周期刷新、应用调度器刷新 URL 集合、以及更通用的 Selector 策略仍属后续工作。
> - 详细差异与未尽项见 §2 和 §15。
...
历史上框架缺少一个面向 URL 的状态查询层：上层无法只传入一组 Tunnel URL 并获得统一的可达性、RTT、失败原因和排序结果，只能通过真正打开 stream 或调用协议私有能力间接判断。当前实现已经补齐 `TunnelManager` 内部状态层、协议 prober、控制面批量查询入口，以及 `forward least_time` 对 URL history 的消费。
...
> 本节描述本需求落地后 `cyfs-gateway-lib` 与 Gateway 控制面的当前实际状态；与最初的"实现边界"相比，绝大多数缺口已经补齐。下面同时列出未尽事项，便于后续 Selector/调度器对接时核对。

### 2.1 Tunnel trait 与 prober 抽象

- `Tunnel` trait 本身保持原有动作集（`ping`、`open_stream`、`create_datagram_client` 等），未在该 trait 上扩展 RTT/失败分类语义，避免大规模兼容破坏。
- 新增 `TunnelUrlProber` trait（见 `tunnel_url_status.rs`），由 `TunnelBuilder::url_prober()` 可选返回。状态查询走 prober，与 `Tunnel::ping()` 解耦，避免 `IPTunnel` / `SocksTunnel` 旧 `ping()` 的"假成功"污染状态。
- 已实现 prober 的协议：`rtcp`（`RtcpUrlProber` → `RTcp::probe_url`）、`tcp`（`TcpUrlProber`）、`ptcp`（复用 `TcpUrlProber`）、`tls`（`TlsUrlProber`）、`quic`（`QuicUrlProber`）、`socks`（`SocksUrlProber`，做 SOCKS5 CONNECT 端到端探测）。
- `udp` 通过 `IPTunnelBuilder::new_no_prober()` 注册，`url_prober()` 返回 `None`，状态查询走 `Unsupported` 路径。
- RTCP 端在 `RTcpTunnel::ping_rtt(timeout)` 上新增了带 RTT 测量的 ping 通道，prober 优先在已有 tunnel 上调用，不会新建第二条同 key tunnel。

### 2.2 TunnelManager 已具备的状态查询层

`TunnelManager` 现已新增以下能力（详见 `tunnel_mgr.rs`）：

- 主状态表 `tunnel_history: RwLock<HashMap<String, TunnelUrlHistory>>`，key 为规范化 URL，存储 `TunnelUrlHistory`（含 `current`、`last_reachable`、`last_unreachable`、`recent_rtt_ms`、成功/失败计数、`pinned`、`runtime_tunnel_key` 等）。
- 单 URL/批量 URL 查询 API：`query_tunnel_url_status`、`query_tunnel_url_statuses`，按 §6 规则走"history 新鲜→ in-flight 合并→ 申请 `probe_limiter` 配额→ 启动新 probe"四步。
- in-flight 合并：`in_flight_probes: AsyncMutex<HashMap<String, SharedProbeFuture>>`，使用 `futures::Shared` 让多个并发查询者共享同一 probe 结果，复用者不占新的 `probe_limiter` 配额。
- 并发限制：`probe_limiter: Arc<Semaphore>`，默认 32，可配置。
- TTL：`Reachable` 30s / `Unknown` 10s / `Unreachable` 5s / `Unsupported` 60s（默认值，可配置）。
- 状态来源完整支持 `ExistingTunnel` / `KeepAlive` / `CachedProbe` / `FreshProbe` / `BusinessConnect` / `BuilderValidation` / `Unsupported`。
- 失败分类用 `TunnelFailureReason` 统一前缀（`pre_connect_dns` / `pre_connect_route` / `connect_refused` / `connect_timeout` / `tls_handshake` / `tunnel_open` / `unsupported_scheme` / `other`），`open_stream_by_url` 与 `create_datagram_client_by_url` 在所有失败分支调用 `record_business_failure`，成功路径调用 `record_business_success` 写入实测耗时。
- LRU 淘汰：超过 `max_memory_history_entries` 时按 `updated_at_ms` 升序淘汰非 pinned 条目；`pin_tunnel_url` 可显式钉住关键 URL（keep_tunnel 默认 pin 自己）。
- 失效与清理：`invalidate_tunnel_url_status`、`clear_tunnel_url_status_cache`。
- 落盘：`TunnelStatusStoreConfig::enable_persist` 默认 `true`，构造时同步加载、后台 `flush_loop` 周期写盘（`tokio::spawn` 持 `Weak`，manager drop 后自然退出），落盘前 `redact_url_for_persist` 脱敏 userinfo，写入用 `tmp + rename` 保证原子。`flush_persisted_history` 提供显式 flush 入口。
- 排序：`sort_urls` 实现 `None` / `ReachableFirst` / `RttAscending` / `CallerPriorityThenRtt`，跨 scheme 按"首次出现位置"分组比较，避免跨协议 RTT 直接比；`include_unsupported = false` 时不进入 `sorted_urls` 但仍保留在 `statuses`。

### 2.3 RTCP/RUDP 状态信息已统一暴露

`RTcpInner::probe_url` 已实现：

- 复用 `compute_tunnel_key`（`{local_did}_{remote_did}` 或附加 `|bootstrap=...`）作为 RTCP 侧 tunnel key，并写入 `TunnelUrlStatus.runtime_tunnel_key`；
- 已存在 tunnel 时调用 `ping_rtt` 测 RTT，仅 ping 成功才返回 `Reachable`，否则 `Unreachable`，不再仅凭 map 中存在对象就判可达；
- 没有 tunnel 时尝试 `create_tunnel` + `ping_rtt`，仍以 ping 结果为准；
- `RtcpStack::start_keep_tunnel` 循环把 `get_tunnel` / `ping` 结果写入 URL history，来源 `KeepAlive`，并对该 URL 调用 `pin_tunnel_url` 防止 LRU 淘汰；
- `TunnelManager::record_tunnel_level_event(tunnel_key, ...)` 提供 tunnel-level 事件传播能力，按 `runtime_tunnel_key` 匹配把状态扩散到所有共享同一 RTCP tunnel 的已知 URL history。
- direct attempt 候选地址 250ms stagger 的并发尝试逻辑保留，address-level RTT 仍留在 RTCP 内部，URL 状态只暴露汇总结果。
...
### 2.5 控制面与 Selector/forward 集成现状

- 控制面已支持 `query_tunnel_url_statuses`、`tunnels_probe`、`/tunnels/probe` 三个等价 method 名，参数在 `gateway.rs::QueryTunnelUrlStatusesParams` 中解析并转换为 `TunnelProbeOptions`。
- 控制面请求中的 URL 解析失败会直接返回 `InvalidParams`，不会像库内批量查询那样形成 per-URL 状态；库内 `query_tunnel_url_statuses(&[Url], ...)` 只接收已解析的 `Url`。
- `GatewayControlClient::query_tunnel_url_statuses` 已提供客户端封装；`test_control_server.rs` 覆盖了 unsupported/unknown URL 经控制面返回 `statuses` 且在 `include_unsupported = false` 时不进入 `sorted_urls`。
- `forward least_time` 已接入 `apply_least_time_via_tunnel_mgr()`：执行入口只以 50ms 预算读取已有 URL history，不设置 `force_probe`，失败/超时保留原候选顺序，避免业务请求路径被 probe 成本拖慢。
- 当前尚未实现独立后台任务周期刷新 URL history；应用调度器仍需要自己决定 URL 集合和刷新节奏，`TunnelManager` 不主动发现 relay/direct URL。

## 3. 目标

### 3.1 对上层提供统一查询能力

`TunnelManager` 已新增按 URL 查询状态的统一入口：
...
**实现差异**：实际类型见 `tunnel_url_status.rs::TunnelUrlStatus`：

- `category` 实际为 `Option<ProtocolCategory>`，未识别 scheme 时为 `None`，避免在状态层强行抛出 `UnknownProtocol`。
- 额外字段 `observed_at_ms: u64`，记录该状态快照生成时间（TTL/排序由这个字段而非 `expires_at_ms` 直接计算，`expires_at_ms` 目前预留未填）。
- 额外字段 `runtime_tunnel_key: Option<String>`，由协议 prober 在共享一条物理 tunnel（典型为 RTCP）时填入，使 `TunnelManager` 能用这个 key 把 tunnel-level 事件传播到所有共享 URL history。
...
**当前实现细节**：`sort_urls` 先按状态 rank 排序：`Reachable`、`Unknown/Probing`、`Unreachable`、`Unsupported`。`RttAscending` 与 `CallerPriorityThenRtt` 会按输入中 scheme 的首次出现位置做跨 scheme 分组，只在同 scheme 组内比较 RTT；`CallerPriorityThenRtt` 的 caller priority 数组与输入 URL 数组按位置对应，缺失项按 `u32::MAX` 处理。
...
**实现差异**：当前实现直接把 `runtime_tunnel_key: Option<String>` 作为 `TunnelUrlHistory` 的字段，而不是单独维护 `runtime_tunnel_index: HashMap<String, RuntimeTunnelRef>`。tunnel-level 事件传播通过 `record_tunnel_level_event(tunnel_key, ...)` 在 `tunnel_history` 上线性扫描匹配 `runtime_tunnel_key` 的条目实现。这种简化在当前 URL 数量级（默认上限 1 万条）下足够，且避免了独立索引的同步开销；如果后续条目规模显著上升，可再补一个 `tunnel_key -> Vec<normalized_url>` 的二级索引以加速查找。`TunnelUrlHistory` 同时新增 `pinned: bool`，用于让 keep_tunnel/Selector 关键 URL 免于 LRU 淘汰。
...
- 若对应 scheme 没有 prober，返回并记录 `Unsupported`；
- 在超时控制内执行 probe；
- 记录并返回结果。
...
- 库内接口接收的是 `&[Url]`，因此 URL 解析失败由调用方或控制面提前处理；控制面当前返回 `InvalidParams`；
...
长期看，`Tunnel::ping()` 应返回 RTT 或底层状态，但短期不强制修改现有 trait，以避免大范围破坏。当前实现通过独立 prober 收敛语义：RTCP 使用 `ping_rtt()`，tcp/ptcp/tls/quic/socks 使用协议专用轻量连接 probe，udp 因无通用可达性语义而返回 `Unsupported`。状态查询不再依赖 `IPTunnel` / `SocksTunnel` 旧 `ping()` 的假成功。

## 8. 状态历史与缓存需求

`TunnelManager` 的主状态表是 URL 历史表。最初设计可以抽象为：
...
**当前实现差异**：代码中没有单独的 `runtime_tunnel_index`，而是在 `TunnelUrlHistory.runtime_tunnel_key` 上保留运行期 tunnel key。`record_tunnel_level_event()` 通过扫描 history 找到共享同一 key 的 URL 并合并状态。`TunnelManagerInner` 还包含 `config: StdMutex<TunnelStatusStoreConfig>` 和 `flush_task_running: AtomicBool`，用于运行期配置与落盘任务控制。
...
当前实现中 `enable_persist` 默认 `true`，但 `persist_path = None` 时加载与 flush 都是 no-op；生产配置需要显式提供路径才会真正写盘。写盘使用 JSON、`tmp + rename` 原子替换，保存前会用 `redact_url_for_persist` 脱敏 userinfo；带 userinfo 的条目重启后会以脱敏 key 加载，这是有意的安全取舍。
...
当前 `RTcpTunnel::ping()` 仍只发送 Ping 包，不等待 Pong，因此不能直接作为 RTT 依据；RTT 状态查询使用新增的 `RTcpTunnel::ping_rtt(timeout)`。`ping_rtt` 会注册 seq 对应的 `pong_waiters`，发送 Ping 后等待匹配 Pong，并在成功、超时、发送失败或 tunnel drop 时清理 waiter。
...
当前按完整 URL 测试目标 CONNECT，因为多链路排序关心的是这条 URL 到目标服务是否可达，而不只是代理本身是否在线。
...
本需求的核心能力在 `cyfs-gateway-lib` 内实现；Gateway 控制面已经做了薄包装。当前支持的 method 名：
...
控制面只负责登录态/权限边界、参数校验和 JSON 序列化，不重新实现 probe 逻辑。当前 URL 解析失败会返回 `InvalidParams`；每个已解析 URL 的 probe 失败才会在库内转换为对应 per-URL 状态。

Selector/forward 不应在每个业务请求路径上同步触发 fresh probe。当前 `least_time` 集成采用如下约束：

- `apply_least_time_via_tunnel_mgr()` 只读取已有 history，`force_probe = false`；
- 单次 lookup 预算固定为 50ms，超时或失败退化为原候选顺序；
- 只有控制面显式诊断、配置变更后的刷新任务，或应用调度器主动刷新 URL 集合时，才应触发 `force_probe`。

后续若增加后台刷新任务，应继续复用同一批量查询接口，并确保刷新任务只处理配置或调度器显式给出的 URL 集合。
...
- `least_time` 从 tunnel_mgr history 读取 RTT 排序，且非 `least_time` plan 保持 no-op。
...
当前 `test_control_server.rs` 已覆盖登录后调用 `query_tunnel_url_statuses`，并验证 unsupported/unknown URL 的 `statuses` 与 `sorted_urls` 行为。后续还应补：

- 无登录或权限不足时拒绝状态查询；
- 非法 URL 返回 `InvalidParams`；
- `sort` 非法值返回 `InvalidParams`；
- `caller_priorities` 与 URL 数量不一致时的兼容行为；
- `/tunnels/probe` 与 `tunnels_probe` alias 的兼容性。

## 15. 分阶段落地建议

> 进度（截至 2026-04-30）：阶段一/二/三已完成；阶段四已部分完成，控制面 API 与 `forward least_time` 已落地，后台刷新和应用调度器集成待补。

### 阶段一：框架、URL history 与缓存（已完成）
...
### 阶段四：控制面与 Selector/forward 集成（部分完成）

- 已完成：控制面增加 `/tunnels/probe` 类 API，并复用 `TunnelManager::query_tunnel_url_statuses`；
- 已完成：`forward least_time` 使用批量状态查询结果做 RTT 重排；
- 已完成：业务请求路径不触发 `force_probe`，lookup 有 50ms 预算并可退化为原顺序；
- 待补：Gateway 或应用调度器后台周期刷新 URL history；
- 待补：除 `least_time` 外，更通用的 Selector 策略如何消费状态、成本和失败历史；
- 约束保持不变：Failover 只在配置允许 URL 集合内切换，Selector 不做设备搜索或 relay 自动发现。
