...
# build_all 会构建 Linux 产物、复制 BNS Foundry 工程，并运行 make_sn_config.ts --seed-v2。
...
不需要重建 VM。当前 `web3-gateway` app 没有定义 `update` command，先不要用
`buckyos-devtest update`；可靠路径是重新 `install` 后复用 VM 内已有 Anvil 合约并重启：
