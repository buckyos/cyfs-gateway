...
mod tests {
    ...
    #[test]
    fn test_gateway_server_capabilities() {
        let registry = cyfs_gateway_app_lib::build_default_gateway_server_registry().unwrap();
        assert_eq!(
            registry.registered_server_types(),
            cyfs_gateway_app_lib::DEFAULT_GATEWAY_SERVER_TYPES.map(str::to_string)
        );
    }
