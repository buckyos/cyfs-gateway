...
impl SnServerFactory {
    ...
    fn parse_pinned_ed25519_public_key(value: &str, field: &str) -> ServerResult<[u8; 32]> {
        use base64::engine::general_purpose::URL_SAFE_NO_PAD;
    ...
    fn default_local_s2s_identity(app_did: Option<&str>) -> ServerResult<SnLocalS2sIdentity> {
    ...
    async fn build_remote_krpc(
        ...
        match Self::resolve_remote_transport_security(provider, field)? {
            ResolvedRemoteTransportSecurity::Tls => {
                ...
                let client =
                    kRPC::new_with_transport(endpoint.as_str(), None, KrpcTransportSecurity::Tls)
                        .await
                        .map_err(|error| {
                            server_err!(
                                ServerErrorCode::InvalidConfig,
    ...
    async fn spawn_test_http_server_with_dst(
        ...
        std::thread::spawn(move || {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build test http runtime");
            let local = tokio::task::LocalSet::new();
            local.block_on(&runtime, async move {
                let listener = TcpListener::from_std(listener).expect("adopt test http listener");
                loop {
                    let (stream, _) = listener.accept().await.unwrap();
                    let http_server = http_server.clone();
                    let stream_info =
                        StreamInfo::new(addr.to_string()).with_dst_addr(dst_addr.clone());
                    tokio::task::spawn_local(async move {
                        let ret =
                            hyper_serve_http(Box::new(stream), http_server, stream_info).await;
                        if let Err(e) = ret {
...
"#;
    ...
    fn auth_and_device_backends_are_selected_independently() {
        for (auth_db, device_info_db) in [
            ...
            (Some("https://auth:8443"), Some("https://device:8443")),
        ] {
            let config: SNServerConfig = serde_json::from_value(json!({
                "id": "test",
                "host": "buckyos.ai",
    ...
    async fn remote_provider_supports_tls_plaintext_and_identity_backed_http_s2s() {
        ...
        assert!(SnServerFactory::build_remote_krpc(
            &https,
            None,
            crate::normalize_sn_auth_db_url,
            crate::normalize_sn_auth_db_s2s_url,
        ...
        assert_eq!(
            SnServerFactory::resolve_remote_transport_security(&http_plaintext, "auth_db").unwrap(),
            ResolvedRemoteTransportSecurity::Plaintext
        );
        assert!(SnServerFactory::build_remote_krpc(
            &http_plaintext,
