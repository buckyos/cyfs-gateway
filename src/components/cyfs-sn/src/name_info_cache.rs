...
impl NameInfoCache {
    ...
    pub fn remove_matching_domains(&self, domains: &[String]) {
        if domains.is_empty() {
            return;
        }
        ...
        let domains = domains
            .iter()
            .map(|domain| Self::normalize_name(domain))
        ...
        self.write_items().retain(|key, _| {
            !domains
                .iter()
                .any(|domain| key.name == *domain || key.name.ends_with(&format!(".{domain}")))
        });
    }
