...
// global
lazy_static! {
    pub static ref GLOBAL_SN_DB: Arc<Mutex<SnDB>> = Arc::new(Mutex::new(SnDB::new().unwrap()));
}
...
impl SnDB {
    ...
    pub fn get_activation_codes(&self) -> Result<Vec<String>> {
        let mut stmt = self
            .conn
            .prepare("SELECT code FROM activation_codes WHERE used = 0")?;
    ...
    pub fn insert_activation_code(&self, code: &str) -> Result<()> {
        let mut stmt = self
            .conn
            .prepare("INSERT INTO activation_codes (code, used) VALUES (?1, 0)")?;
    ...
    pub fn check_active_code(&self, active_code: &str) -> Result<bool> {
        let mut stmt = self
            .conn
            .prepare("SELECT used FROM activation_codes WHERE code =?1")?;
        let used: Result<Option<i32>, rusqlite::Error> =
            stmt.query_row(params![active_code], |row| row.get(0));
        if used.is_err() {
            return Ok(false);
        }
    ...
    pub fn register_user_with_sn_ips(
        &self,
        active_code: &str,
        username: &str,
        public_key: &str,
        ...
        if let Some(0) = used {
            ...
            let mut stmt = self
                .conn
                .prepare("UPDATE activation_codes SET used = 1 WHERE code =?1")?;
    ...
    pub fn register_user_directly(
        ...
        let mut stmt = self.conn.prepare("INSERT INTO users (username, state, public_key, activation_code, zone_config, user_domain,self_cert, sn_ips) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)")?;
        stmt.execute(params![
            username,
            UserState::Active.to_string(),
            public_key,
    ...
    pub fn update_user_zone_config(&self, username: &str, zone_config: &str) -> Result<()> {
        let mut stmt = self
            .conn
            .prepare("UPDATE users SET zone_config =?1 WHERE username =?2")?;
    ...
    pub fn update_user_sn_ips(&self, username: &str, sn_ips: &str) -> Result<()> {
        let mut stmt = self
            .conn
            .prepare("UPDATE users SET sn_ips =?1 WHERE username =?2")?;
    ...
    pub fn update_user_self_cert(&self, username: &str, self_cert: bool) -> Result<()> {
        let mut stmt = self
            .conn
            .prepare("UPDATE users SET self_cert =?1 WHERE username =?2")?;
    ...
    pub fn get_user_sn_ips(&self, username: &str) -> Result<Option<String>> {
        let mut stmt = self
            .conn
            .prepare("SELECT sn_ips FROM users WHERE username =?1")?;
        let sn_ips = stmt
    ...
    pub fn update_device_by_did(&self, did: &str, ip: &str, description: &str) -> Result<()> {
        ...
        let mut stmt = self
            .conn
            .prepare("UPDATE devices SET ip =?1, description =?2, updated_at =?3 WHERE did =?4")?;
        stmt.execute(params![ip, description, now, did])?;
        Ok(())
    }
    pub fn update_device_by_name(
        &self,
        username: &str,
        device_name: &str,
        did: &str,
    ...
    pub fn update_device_info_by_name(
        &self,
        username: &str,
        device_name: &str,
        ip: &str,
    ...
    pub fn query_device_by_name(
        &self,
        username: &str,
        device_name: &str,
    ) -> Result<Option<SNDeviceInfo>> {
        ...
        let device_info = stmt
            .query_row(params![device_name, username], |row| {
                Ok(SNDeviceInfo {
                    owner: row.get(0)?,
...
mod tests {
    ...
    async fn test_main() -> Result<()> {
        ...
        if let Some(device_info) =
            db.query_device("did:dev:gubVIszw-u_d5PVTh-oc8CKAhM9C-ne5G_yUK5BDaXc")?
        {
            println!("\n=== Device Info (by DID) ===");
