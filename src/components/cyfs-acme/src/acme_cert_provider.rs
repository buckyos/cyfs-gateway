...
pub struct AcmeCertProvider {
    id: String,
    acme: AcmeCertManagerRef,
}

impl AcmeCertProvider {
    pub fn new(id: impl Into<String>, acme: AcmeCertManagerRef) -> Arc<Self> {
        Arc::new(Self {
            id: id.into(),
            acme,
        })
    ...
    pub fn acme_manager(&self) -> AcmeCertManagerRef {
        self.acme.clone()
    }
}

impl Debug for AcmeCertProvider {
...
impl CertProvider for AcmeCertProvider {
    ...
    fn add_request(&self, request: CertRequest) -> Result<()> {
        self.acme.add_acme_item(AcmeItem::new(
            request.domain,
    ...
    fn resolve_client_cert(&self, domain: &str) -> Result<Option<CertMaterial>> {
        Ok(self
            ...
            .and_then(|stub| stub.get_material()))
    }

    fn get_status(&self, domain: &str) -> CertStatus {
        let Some(stub) = self.acme.get_cert_by_host(domain) else {
            return CertStatus {
        ...
        let material = stub.get_material();
        let last_error = stub.get_last_error();
        let state = if material.is_some() {
            match stub.get_state() {
                CertStubState::Renewing => CertStatusState::Renewing,
                _ => CertStatusState::Ready,
            }
        } else {
            match stub.get_state() {
                CertStubState::Pending => {
                    if last_error.is_some() {
                        CertStatusState::Error
                    } else {
                        CertStatusState::Pending
                    }
                }
                CertStubState::Ready => CertStatusState::Error,
                CertStubState::Renewing => CertStatusState::Renewing,
                CertStubState::Expired => CertStatusState::Expired,
            }
        };
    ...
    fn get_http01_auth(&self, token: &str) -> Option<String> {
        self.acme.get_auth_of_token(token)
    }
}
