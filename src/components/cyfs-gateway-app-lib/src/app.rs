...
use crate::{
    get_default_config_path, load_config_from_file, run_server_tempalte_pkg, GatewayAppProfile,
    GatewayComposition, GatewayConfigParser, GatewayFactory, GatewayParams,
};
pub use crate::{
    merge, run_debug_command, AcmeConfig, AcmeHostConfig, AcmeHttpChallengeServerConfigParser,
    ConfigMerger, CyfsDirServerConfigParser, DirServerConfigParser, GatewayControlServer,
    GatewayControlServerConfig, GatewayControlServerConfigParser, GatewayControlServerContext,
    GatewayControlServerFactory, GatewayProcessChainDoc, HttpServerConfigParser,
    QuicStackConfigParser, RtcpStackConfigParser, TcpStackConfigParser, TlsCA,
    TlsStackConfigParser, UdpStackConfigParser, GATEWAY_CONTROL_SERVER_CONFIG,
...
static ACTIVE_APP_PROFILE: OnceLock<GatewayAppProfile> = OnceLock::new();

fn active_app_profile() -> &'static GatewayAppProfile {
    ACTIVE_APP_PROFILE
        .get()
        .expect("gateway app profile must be installed before CLI dispatch")
}

fn install_active_app_profile(profile: GatewayAppProfile) -> Result<()> {
    if let Some(existing) = ACTIVE_APP_PROFILE.get() {
        if existing == &profile {
            return Ok(());
        }
        return Err(anyhow!(
            "gateway app profile is already installed as '{}'",
            existing.binary_name
        ));
    }
    ACTIVE_APP_PROFILE
        .set(profile)
        .map_err(|_| anyhow!("failed to install gateway app profile"))
}

impl GatewayApp {
    pub fn new(profile: GatewayAppProfile, composition: GatewayComposition) -> Result<Self> {
        if composition.profile() != &profile {
            return Err(anyhow!(
                "gateway composition profile '{}' does not match app profile '{}'",
                composition.profile().binary_name,
                profile.binary_name
            ));
        }
        Ok(Self {
...
pub async fn gateway_service_main(
    ...
    install_active_app_profile(app.profile)?;
    let loaded_config = load_config_from_file(app.profile(), config_file).await?;
    debug!(
        "Gateway config: {}",
...
async fn run_gateway_with_config(
    ...
    let composition = app.composition().clone();
    let manifest = composition.manifest();
    info!(
        "Installed gateway modules: [{}]",
        manifest.modules.join(", ")
    );
    info!(
        "Registered gateway server types: [{}]",
        manifest.servers.join(", ")
    );
    info!(
        "Registered gateway stack protocols: [{}]",
        manifest.stacks.join(", ")
    );
    let parser = Arc::new(GatewayConfigParser::new(composition));
    ...
    if gateway_config.device_manager.enabled {
        ...
        let device_online_db_path =
            get_buckyos_service_data_dir(app.profile().service_data_namespace)
...
fn get_config_file_path(profile: &GatewayAppProfile, matches: &clap::ArgMatches) -> PathBuf {
...
pub fn read_login_token(server: &str) -> Option<String> {
    let profile = active_app_profile();
    let data_dir = get_buckyos_service_data_dir(profile.service_data_namespace).join("token_key");
    let token_dir = get_buckyos_service_data_dir(profile.service_data_namespace).join("cli_token");
    if !token_dir.exists() {
...
fn save_login_token(server: &str, token: &str) {
    ...
    let token_dir =
        get_buckyos_service_data_dir(active_app_profile().service_data_namespace).join("cli_token");
    let token_file = token_dir.join(hex::encode(server.to_lowercase()));
...
fn load_local_tools(profile: &GatewayAppProfile) -> Vec<LocalToolCommand> {
...
async fn run_template_local(app: &GatewayApp, template_id: &str, args: Vec<String>) -> Result<()> {
    let template_dir = get_buckyos_system_etc_dir()
        .join(app.profile().template_namespace)
...
pub async fn gateway_app_main(app: &GatewayApp) -> Result<()> {
    install_active_app_profile(app.profile)?;
    let mut command = Command::new(app.profile().display_name)
        ...
        .subcommand(Command::new("add_rule")
            .about("Add a rule")
            .after_help("Examples:\n  cyfs_gateway add_rule stack:s1:main \"http-probe && call-server www;\"".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("rule")
        ...
        .subcommand(Command::new("append_rule")
            ...
            .after_help("Examples:\n  cyfs_gateway append_rule stack:s1:main \"eq ${REQ.host} \\\"a.com\\\" && call-server a;\"".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("rule")
        ...
        .subcommand(Command::new("insert_rule")
            ...
            .after_help("Examples:\n  cyfs_gateway insert_rule stack:s1:main 2 \"rewrite /old /new;\"".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("pos")
        ...
        .subcommand(Command::new("move_rule")
            ...
            .after_help("Examples:\n  cyfs_gateway move_rule stack:s1:main 1\n  cyfs_gateway move_rule stack:s1:main:b1:2 3".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("new_pos")
        ...
        .subcommand(Command::new("set_rule")
            ...
            .after_help("Examples:\n  cyfs_gateway set_rule stack:s1:main:b1 \"forward \\\"tcp:///1.1.1.1:80\\\";\"\n  cyfs_gateway set_rule stack:s1:main:b1:2 \"rewrite /old /new;\"".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("rule")
        ...
        .subcommand(Command::new("add-name-provider")
            ...
            .after_help("Examples:\n  cyfs_gateway add-name-provider http://127.0.0.1:8080\n  cyfs_gateway add-name-provider https://resolver.example.com --trust-level 100".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("url")
        ...
        .subcommand(Command::new("add_dispatch")
            ...
            .after_help("Examples:\n  cyfs_gateway add_dispatch 18080 192.168.0.1:1900\n  cyfs_gateway add_dispatch 0.0.0.0:8080 10.0.0.1:9000 --protocol udp".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("local")
        ...
        .subcommand(Command::new("remove_dispatch")
            ...
            .after_help("Examples:\n  cyfs_gateway remove_dispatch 18080\n  cyfs_gateway remove_dispatch 0.0.0.0:8080 --protocol udp".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("local")
                .help("local endpoint, such as 18080 or 0.0.0.0:18080")
                .required(true))
            .arg(Arg::new("protocol")
        ...
        .subcommand(Command::new("add_router")
            ...
            .after_help("Examples:\n  cyfs_gateway add_router --uri /sn --target /www\n  cyfs_gateway add_router --uri /api --target http://127.0.0.1:9000/ --id server:api:main".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .long("id")
        ...
        .subcommand(Command::new("remove_router")
            ...
            .after_help("Examples:\n  cyfs_gateway remove_router --id api_router --uri /api --target http://127.0.0.1:9000/".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .long("id")
        ...
        .subcommand(Command::new("remove_rule")
            .about("Delete a rule")
            .after_help("Examples:\n  cyfs_gateway remove_rule stack:s1:main:b1\n  cyfs_gateway remove_rule stack:s1:main:b1:2".replace("cyfs_gateway", app.profile().binary_name))
            .arg(Arg::new("id")
                .help("rule id")
                .required(true))
            .arg(Arg::new("server")
    ...
    let local_tools = load_local_tools(app.profile());
    ...
    init_logging(app.profile().log_name, is_service);
    ...
    match matches.subcommand() {
        ...
        Some(("process_chain", sub_matches)) => {
            let doc = match GatewayProcessChainDoc::new(app.profile(), app.composition()) {
                Ok(doc) => doc,
                Err(e) => {
        ...
        Some(("debug", sub_matches)) => {
            ...
            match run_debug_command(app.profile(), req_file.as_str(), config_file, id, repeat).await
            {
                Ok(_) => {
                    std::process::exit(0);
                }
        ...
        Some(("run", _sub_matches)) => {
            ...
            info!("{} start...", app.profile().binary_name);
            ...
            match run_template_local(app, template_id.as_str(), run_args.args).await {
                Ok(_) => {
                    std::process::exit(0);
                }
                Err(e) => {
    ...
    info!("{} service start...", app.profile().display_name);
    ...
    let config_file = get_config_file_path(app.profile(), &matches);
    ...
    gateway_service_main(app, config_file.as_path(), params).await
}
