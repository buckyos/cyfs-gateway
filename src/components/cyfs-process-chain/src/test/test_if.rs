...
"#;
    ...
    assert_eq!(
        execute_with_req(script, "visitor", "https", "yes").await?,
        "guest"
    );
    Ok(())
}
...
"#;
    ...
    assert_eq!(
        execute_with_req(script, "user", "http", "yes").await?,
        "reject"
    );
    Ok(())
}
...
"#;
    ...
    assert_eq!(
        execute_with_req(script, "user", "https", "no").await?,
        "nested_no"
    );
    Ok(())
}
