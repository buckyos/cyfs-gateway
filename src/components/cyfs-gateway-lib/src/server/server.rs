...
pub async fn serve_http_by_rpc_handler<T: RPCHandler + Send + Sync + 'static>(
    ...
    Response::builder().body(full_body(body_json)).map_err(|e| {
        server_err!(
...
fn client_ip(
    info: &StreamInfo,
) -> Result<IpAddr, http::Response<UnsyncBoxBody<Bytes, ServerError>>> {
...
impl HttpResponseHeaderMap {
    ...
    pub fn into_response(
        self,
    ) -> Result<http::Response<UnsyncBoxBody<Bytes, ServerError>>, String> {
