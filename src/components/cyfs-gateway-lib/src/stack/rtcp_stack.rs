...
- id: main
  ...
  blocks:
    ...
    #[tokio::test]
    async fn test_rtcp_stack_nested_remote_rebinds_transport_via_rtcp_relay() {
        let _ = init_name_lib_for_test(&HashMap::new()).await;
    ...
    - id: main
      block: |
        ...
        server_manager
            ...
            .unwrap();
        let tunnel_manager_a = TunnelManager::new();
        let connection_manager = ConnectionManager::new();
        let stack_context = build_stack_context(
            server_manager,
            tunnel_manager_a.clone(),
            Arc::new(DefaultLimiterManager::new()),
            StatManager::new(),
            Some(Arc::new(GlobalProcessChains::new())),
        );
        let stack_a = RtcpStack::builder()
            .id("test-a")
            .bind("127.0.0.1:2992".to_string())
            .device_config(device_config.clone())
            .private_key(pkcs8_bytes)
            .hook_point(chains)
            .connection_manager(connection_manager.clone())
    ...
    - id: main
      block: |
        ...
        server_manager
            ...
            .unwrap();
        let tunnel_manager_b = TunnelManager::new();
        let stack_context = build_stack_context(
            server_manager,
            tunnel_manager_b.clone(),
            Arc::new(DefaultLimiterManager::new()),
            StatManager::new(),
            Some(Arc::new(GlobalProcessChains::new())),
        );
        let stack_b = RtcpStack::builder()
            .id("test-b")
            .bind("127.0.0.1:2994".to_string())
            .device_config(device_config.clone())
            .private_key(pkcs8_bytes)
            .hook_point(chains)
            .connection_manager(connection_manager.clone())
...
- id: main
  ...
  blocks:
    - id: main
      block: |
        return "forward tcp:///127.0.0.1:2994";
        "#;
        ...
        let stack_c = RtcpStack::builder()
            .id("test-c")
            .bind("127.0.0.1:2993".to_string())
            .device_config(device_config.clone())
            .private_key(pkcs8_bytes)
            .hook_point(chains)
            .connection_manager(connection_manager.clone())
            ...
            .await
            .unwrap();
        stack_c.start().await.unwrap();

        tokio::time::sleep(std::time::Duration::from_secs(1)).await;

        let bootstrap_url = format!("rtcp://{}:2993/bootstrap:1", id_c.to_host_name());
        let encoded_bootstrap = percent_encoding::utf8_percent_encode(
            &bootstrap_url,
            percent_encoding::NON_ALPHANUMERIC,
        )
        .to_string();
        let nested_remote_tunnel_url = Url::parse(
            format!("rtcp://{}@{}:2994", encoded_bootstrap, id_b.to_host_name()).as_str(),
        )
        .unwrap();

        let tunnel = tokio::time::timeout(
            std::time::Duration::from_secs(10),
            tunnel_manager_a.get_tunnel(&nested_remote_tunnel_url, None),
        )
        .await
        .expect("nested remote tunnel creation timed out")
        .expect("A should build the outer tunnel to B through C");

        tokio::time::timeout(std::time::Duration::from_secs(10), tunnel.ping())
            .await
