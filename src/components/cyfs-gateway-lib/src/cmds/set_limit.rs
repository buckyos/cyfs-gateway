...
use cyfs_process_chain::{
    ...
    ExternalCommand, MemoryMapCollection, command_help,
};
...
impl ExternalCommand for SetLimit {
    ...
    fn check(&self, args: &CommandArgs) -> Result<(), String> {
        ...
        let _limiter_id = matches
            .get_one::<String>("limiter_id")
            .map(|s| s.to_string());
        let down_speed = matches
            .get_one::<String>("down_speed")
            .map(|s| s.to_string());
        let upload_speed = matches
            .get_one::<String>("upload_speed")
            .map(|s| s.to_string());
        match count {
            3 => {
                ...
                if let Some(down_speed_str) = &down_speed {
                    if !is_valid_speed(down_speed_str) {
                        return Err(format!(
                            "Invalid download speed format: {}. Expected format: 100.1KB/s. Supports units: B/s, KB/s, MB/s, GB/s",
                            down_speed_str
                        ));
                    }
                }
                if let Some(upload_speed_str) = &upload_speed {
                    if !is_valid_speed(upload_speed_str) {
                        return Err(format!(
                            "Invalid upload speed format: {}. Expected format: 100.1KB/s. Supports units: B/s, KB/s, MB/s, GB/s",
                            upload_speed_str
                        ));
                    }
                }
    ...
    async fn exec(
        ...
        match count {
            3 => {
                ...
                map.insert("limiter_id", CollectionValue::String(limiter_id))
                    .await?;
                ...
                map.insert(
                    "down_speed",
                    CollectionValue::String(format!("{}", down_speed)),
                )
                .await?;
                ...
                map.insert(
                    "upload_speed",
                    CollectionValue::String(format!("{}", upload_speed)),
                )
                .await?;
            }
            2 => {
        ...
        context
            .env()
            .create("LIMIT", CollectionValue::Map(map), EnvLevel::Global)
            .await?;
        Ok(CommandResult::success_with_string("LIMIT"))
    }
}
