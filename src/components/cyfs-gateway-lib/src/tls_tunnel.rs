...
const TLS_TUNNEL_OPTION_CLIENT_CERT: &str = "client_cert";
const TLS_TUNNEL_OPTION_SNI: &str = "sni";
const TLS_TUNNEL_OPTION_INSECURE: &str = "insecure";

#[derive(Clone, Default)]
struct TlsTunnelOptions {
    client_cert_alias: Option<String>,
    sni: Option<String>,
    insecure: bool,
}

impl TlsTunnelOptions {
    fn from_tunnel_options(options: Option<&TunnelOptions>) -> Self {
        let client_cert_alias = options
            .and_then(|options| options.get(TLS_TUNNEL_OPTION_CLIENT_CERT))
            .map(str::to_owned);
        let sni = options
            .and_then(|options| options.get(TLS_TUNNEL_OPTION_SNI))
            .map(str::to_owned);
        let insecure = options
            .and_then(|options| options.get(TLS_TUNNEL_OPTION_INSECURE))
            .map(|value| {
                value.is_empty()
                    || matches!(
...
impl TlsTunnel {
    pub fn new(
        ...
        let tls_options = TlsTunnelOptions::from_tunnel_options(options.as_ref());
        TlsTunnel {
            options,
            tls_options,
            client_cert_manager,
    ...
    async fn open_stream_by_dest(
        ...
        let builder =
            ClientConfig::builder_with_provider(Arc::new(rustls::crypto::ring::default_provider()))
                ...
                .map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        let mut config = if self.tls_options.insecure {
            let builder = builder
                .dangerous()
                .with_custom_certificate_verifier(Arc::new(NoCertificateVerifier));
            if let Some(alias) = self.tls_options.client_cert_alias.as_deref() {
                let material = self
                    .client_cert_manager
